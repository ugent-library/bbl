...
// Provides both bare DC and OAI-DC (OAI-PMH wrapper) encoders.
package dcformat
...
// WorkEncoder encodes a single work as bare Dublin Core XML.
type WorkEncoder struct{}

func (e *WorkEncoder) Encode(work *bbl.Work) ([]byte, error) {
	var buf bytes.Buffer
	buf.WriteString(`<dc xmlns:dc="http://purl.org/dc/elements/1.1/">`)
	writeFields(&buf, work)
	buf.WriteString(`</dc>`)
	return buf.Bytes(), nil
}

// OAIWorkEncoder encodes a single work as OAI Dublin Core XML.
type OAIWorkEncoder struct{}

func (e *OAIWorkEncoder) Encode(work *bbl.Work) ([]byte, error) {
	var buf bytes.Buffer
	buf.WriteString(oaiDCStart)
	writeFields(&buf, work)
	buf.WriteString(`</oai_dc:dc>`)
	return buf.Bytes(), nil
}

func writeFields(buf *bytes.Buffer, work *bbl.Work) {
...
var (
	...
	_ bbl.WorkEncoder = (*OAIWorkEncoder)(nil)
)
