...
type ReindexWorks struct {
	river.WorkerDefaults[jobs.ReindexWorks]
	repo  *pgxrepo.Repo
	index bbl.Index
}

func NewReindexWorks(repo *pgxrepo.Repo, index bbl.Index) *ReindexWorks {
	return &ReindexWorks{
		repo:  repo,
		index: index,
	}
...
func (w *ReindexWorks) Work(ctx context.Context, job *river.Job[jobs.ReindexWorks]) error {
