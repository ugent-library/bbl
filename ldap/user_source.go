...
type Mapping struct {
	SourceRecordID string            `toml:"source_record_id"`
	Username       string            `toml:"username"`
	Email          string            `toml:"email"`
	Name           string            `toml:"name"`
	Role           string            `toml:"role"`
	Identifiers    map[string]string `toml:"identifiers"` // scheme → attr
}
...
func (s *UserSource) attrs() []string {
	...
	var attrs []string
	for _, a := range []string{m.SourceRecordID, m.Username, m.Email, m.Name, m.Role} {
		if a != "" && !seen[a] {
			seen[a] = true
...
func (s *UserSource) mapEntry(raw map[string][]string) *bbl.ImportUserInput {
	...
	in := &bbl.ImportUserInput{
		SourceRecordID: first(raw[m.SourceRecordID]),
