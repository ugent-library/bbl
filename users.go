...
// $1=source, $2=source_id, $3=username, $4=email, $5=name, $6=role,
...
const importUserSQL = `
	...
	updated AS (
		...
		RETURNING id, created_at, username, email, name, role, deactivate_at, person_id, auth_providers
	),
	created AS (
	...
	source_stamp AS (
		INSERT INTO bbl_user_sources (user_id, source, source_id, expires_at)
...
func (r *Repo) GetUser(ctx context.Context, id ID) (*User, error) {
	row := r.db.QueryRow(ctx, `
		SELECT id, created_at, username, email, name, role, deactivate_at, person_id, auth_providers
		FROM bbl_users
		WHERE id = $1`, id)
	u, err := scanUser(row)
...
func (r *Repo) GetUserByIdentifier(ctx context.Context, scheme, val string) (*User, error) {
	row := r.db.QueryRow(ctx, `
		SELECT u.id, u.created_at, u.username, u.email, u.name, u.role, u.deactivate_at, u.person_id
		FROM bbl_users u
...
// source is the source name stored on each record; authProvider is optional
// and, if non-empty, is attached to the user's auth providers on creation.
// Returns the number of successfully imported users and the first fatal error.
func (r *Repo) ImportUsers(ctx context.Context, source, authProvider string, seq iter.Seq2[*ImportUserInput, error]) (int, error) {
	const batchSize = 250
	var (
		...
		if _, err := scanUser(results.QueryRow()); err != nil {
			if isUniqueViolation(err) {
				return n, fmt.Errorf("ImportUsers: identifier conflict for source record %s/%s: %w",
					in.Source, in.SourceID, ErrConflict)
			}
